...
function preload() {
  for (i = 0; i < 30; i++) {
    let imgNew = loadImage(`/Talleres/sketches/taller3/assets/i${i + 1}.jpg`);
    imgs.push(imgNew);
  }
  img = loadImage(`/Talleres/sketches/taller3/assets/i${p + 1}.jpg`);
}
...
function keyPressed() {
  if (key == "r") {
    p = (p + 1) % 30;
    img.copy(imgs[p], 0, 0, size, size, 0, 0, size, size);
    img.loadPixels();
  }
}
